...
hexo.extend.helper.register("pageInfo", function (page) {
    ...
    if (sbgUtility.fs.existsSync(cachePath)) {
        try {
            var extract = sbgUtility.jsonParseWithCircularRefs(sbgUtility.fs.readFileSync(cachePath, "utf-8"));
            return extract;
        }
        catch (error) {
...
var require$$3 = /*@__PURE__*/getAugmentedNamespace(browser);
...
function requireDist () {
	...
	dist.deepmerge = deepmerge;
	dist.deepmergeCustom = deepmergeCustom;
	dist.deepmergeInto = deepmergeInto;
	dist.deepmergeIntoCustom = deepmergeIntoCustom;
	dist.getKeys = getKeys;
	dist.getObjectType = getObjectType;
	dist.objectHasProperty = objectHasProperty;
	return dist;
}
...
function requireConfig () {
	...
	const { deepmerge } = /*@__PURE__*/ requireDist();
